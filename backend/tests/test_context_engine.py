...
class TestWorkoutLifecycle:
    ...
    async def test_new_user_has_no_active_workout(self, context_engine: ContextEngine, user_id: str):
        """A new user should not have an active workout by default."""
        ctx = await context_engine.get_context(user_id)
        
        assert ctx["workout_active"] is False

    async def test_start_workout_activates_session(self, context_engine: ContextEngine, user_id: str):
        """Starting a workout should mark the session as active."""
        await context_engine.start_workout(user_id, routine_name="Push Day")
        
        ctx = await context_engine.get_context(user_id)
        assert ctx["workout_active"] is True
        assert ctx["current_routine"] == "Push Day"

    async def test_start_workout_initializes_set_counter_to_one(
        self, context_engine: ContextEngine, user_id: str
    ):
        """The set counter should start at 1 when workout begins."""
        await context_engine.start_workout(user_id)
        
        ctx = await context_engine.get_context(user_id)
        assert ctx["current_set"] == 1

    async def test_end_workout_deactivates_session(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Ending a workout should deactivate the session."""
        await context_engine.end_workout(user_with_active_workout)
        
        ctx = await context_engine.get_context(user_with_active_workout)
        assert ctx["workout_active"] is False

    async def test_end_workout_returns_summary_with_completed_exercises(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Ending a workout should return a summary of completed exercises."""
        summary = await context_engine.end_workout(user_with_active_workout)
        
        assert "exercises_completed" in summary
        assert "completed_exercises" in summary
        assert "duration" in summary

    async def test_end_workout_without_start_returns_empty_summary(
        self, context_engine: ContextEngine, user_id: str
    ):
...
class TestExerciseNavigation:
    ...
    async def test_get_current_exercise_returns_first_planned_exercise(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Should return the first exercise when workout starts."""
        exercise = await context_engine.get_current_exercise(user_with_active_workout)
        
        assert exercise is not None
        assert exercise["name"] == "Bankdrücken"

    async def test_get_current_exercise_returns_none_when_no_workout(
        self, context_engine: ContextEngine, user_id: str
    ):
        """Should return None if no workout is active."""
        exercise = await context_engine.get_current_exercise(user_id)
        
        assert exercise is None

    async def test_next_exercise_advances_to_second_exercise(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Moving to next exercise should advance the index."""
        await context_engine.next_exercise(user_with_active_workout)
        
        exercise = await context_engine.get_current_exercise(user_with_active_workout)
        assert exercise is not None
        assert exercise["name"] == "Rudern"

    async def test_next_exercise_adds_previous_to_completed_list(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
...
class TestSetRecording:
    ...
    async def test_record_set_increments_set_counter(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Recording a set should increment the set counter."""
        await context_engine.record_set(user_with_active_workout, weight=80, reps=10)
        
        ctx = await context_engine.get_context(user_with_active_workout)
        assert ctx["current_set"] == 2

    async def test_record_set_stores_last_weight(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
    ...
    async def test_multiple_sets_increment_counter_correctly(
        self, context_engine: ContextEngine, user_with_active_workout: str
    ):
        """Multiple sets should increment the counter each time."""
        await context_engine.record_set(user_with_active_workout, weight=80, reps=10)
        await context_engine.record_set(user_with_active_workout, weight=80, reps=8)
        await context_engine.record_set(user_with_active_workout, weight=80, reps=6)
